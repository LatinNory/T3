...
class TensorHelper:
    ...
    def _example_level_pad(self, responses: torch.Tensor,
        ...
        padded_responses_with_step = torch.full(
            (batch_size, seq_len), self.config.pad_token_id,
            dtype=responses.dtype, device=responses.device
        )
        padded_responses[active_mask] = responses
        padded_responses_with_step[active_mask] = responses_ids_with_step
        ...
        return padded_responses, padded_responses_with_step, padded_responses_str