...
TRAIN_PROJECT_NAME="${TRAIN_PROJECT_NAME:-${DATA_NAME}-Train}"
...
for pair in "${checkpoint_steps[@]}"; do
  ...
  MERGED_MODEL_DIR="${CHECKPOINT_ROOT}/${TRAIN_PROJECT_NAME}/${CHECKPOINT}/actor-hf"
  python3 -m verl.model_merger merge \
      --backend fsdp \
      --local_dir "${CHECKPOINT_ROOT}/${TRAIN_PROJECT_NAME}/${CHECKPOINT}/actor/best_step_${STEP}" \
