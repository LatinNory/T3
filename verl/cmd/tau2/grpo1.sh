...
export USE_AREW_BONUS="${USE_AREW_BONUS:-true}"
export AREW_BONUS_MODE="${AREW_BONUS_MODE:-step_abs_sum}"
export AREW_BONUS_SCALE="${AREW_BONUS_SCALE:-25.0}"
export AREW_START_STEP="${AREW_START_STEP:-0}"
export AREW_END_STEP="${AREW_END_STEP:--1}"
export AREW_NEGATIVE_ON_NEUTRAL="${AREW_NEGATIVE_ON_NEUTRAL:-false}"

project_name="${DATA_NAME}-${DOMAIN_NAME}-Train"
experiment_name="solo-grpo-step_abs_sum-arew-25.0-7B-think"
...
python3 -m verl.trainer.main_ppo \
    ...
    +algorithm.arew_negative_on_neutral="${AREW_NEGATIVE_ON_NEUTRAL}" \
    ...
    algorithm.use_arew_bonus="${USE_AREW_BONUS}" \
    algorithm.arew_bonus_mode="${AREW_BONUS_MODE}" \
    algorithm.arew_bonus_scale="${AREW_BONUS_SCALE}" \
    algorithm.arew_start_step="${AREW_START_STEP}" \
    algorithm.arew_end_step="${AREW_END_STEP}" \
    \
